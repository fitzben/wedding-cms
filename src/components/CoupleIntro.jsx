  ...
  useEffect(() => {
    const loadCoupleMedia = async () => {
      try {
        ...
        const coupleSection = sections.find(
          (s) =>
            s.name.toLowerCase() === "couple photos" ||
            s.key === "couple" ||
            s.key === "couple_photos",
        );
        if (coupleSection) {
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-center gap-12 md:gap-4 lg:gap-8 relative">
          ...
          >
            <div
              className={`relative w-64 h-80 md:w-72 md:h-96 mb-6 rounded-3xl p-2 border border-gold/40 transition-all duration-500 group bg-white/50 backdrop-blur-sm cursor-pointer ${hoveredProfile ? "shadow-2xl scale-105" : "shadow-lg"}`}
            >
              <div className="w-full h-full rounded-2xl overflow-hidden relative">
                {coupleMedia[0] ? (
                  <img
                    src={coupleMedia[0].public_url}
            ...
            <p className="text-maroon text-[10px] sm:text-xs uppercase tracking-[0.2em] font-light mb-1">
              {groomName}
            </p>
            <p className="text-charcoal/40 text-[10px] uppercase tracking-[0.2em] font-light mb-4">
              The Groom
            </p>
            ...
            <div className="space-y-1 mb-6">
              ...
              <p className="text-sm font-serif italic text-charcoal/80">
                {settings?.groom_father || "Father Name"} &{" "}
                {settings?.groom_mother || "Mother Name"}
              </p>
            </div>
            ...
            {groomIG && (
              ...
              >
                <Icon
                  icon="mdi:instagram"
                  className="text-lg group-hover/link:-translate-y-1 transition-transform duration-300"
                />
          ...
          <div className="flex items-center justify-center w-full md:w-auto my-12 md:my-0 px-4 md:px-0">
            ...
            <div className="relative flex items-center">
              <div
                className="h-[1px] w-20 md:w-12 lg:w-20 bg-gold/50 obs-hide obs-draw-left"
                style={{ animationDelay: "500ms" }}
              ></div>
              <div
                className="absolute right-0 text-gold/60 anim-pulse-left pointer-events-none"
                style={{ animationDelay: "1.2s" }}
              >
                <Icon
                  icon="material-symbols:astrophotography-mode"
                  className="text-xs"
                />
              </div>
            </div>
            ...
            {/* Heart */}
            <div
              className="mx-4 text-maroon obs-hide obs-heartbeat relative"
            ...
            <div className="relative flex items-center">
              <div
                className="h-[1px] w-20 md:w-12 lg:w-20 bg-gold/50 obs-hide obs-draw-right"
                style={{ animationDelay: "500ms" }}
              ></div>
              <div
                className="absolute left-0 text-gold/60 anim-pulse-right pointer-events-none"
                style={{ animationDelay: "1.2s" }}
              >
                <Icon
                  icon="material-symbols:astrophotography-mode"
          ...
          >
            ...
            <div className="space-y-1 mb-6">
              ...
              <p className="text-sm font-serif italic text-charcoal/80">
                {settings?.bride_father || "Father Name"} &{" "}
                {settings?.bride_mother || "Mother Name"}
              </p>
            </div>
