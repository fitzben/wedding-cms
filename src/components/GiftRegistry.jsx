...
const API = {
  list: () =>
    fetch(`${BASE_URL}/api/gifts/registry`).then(r => r.json()),
  claim: (id, body) =>
...
function ClaimModal({ item, onClose, onSuccess }) {
  const [form, setForm]       = useState({ claimer_name: '', message: '', quantity: 1 });
  ...
  const handleSubmit = async () => {
    if (!form.claimer_name.trim()) { setError('Nama harus diisi'); return; }
    if (form.quantity < 1 || form.quantity > remaining) {
      setError(`Jumlah harus antara 1 – ${remaining}`); return;
    }
      ...
      <div className="relative w-full sm:max-w-md bg-[#1a0a08] rounded-t-2xl sm:rounded-2xl border border-gold/20 shadow-2xl overflow-hidden">
        ...
        {done ? (
          ...
          <div className="p-8 flex flex-col items-center text-center gap-4">
            ...
            <div>
              <p className="font-serif italic text-2xl text-ivory mb-1">Terima kasih!</p>
              <p className="font-sans font-light text-[13px] text-ivory/60 leading-relaxed">
                Kamu sudah claim <strong className="text-ivory">{form.quantity}×</strong> {item.name}.
                {done.quantity_claimed < item.quantity_needed && (
                  <> Masih ada <strong className="text-gold">{item.quantity_needed - done.quantity_claimed}</strong> lagi yang bisa diambil orang lain.</>
                )}
                {done.quantity_claimed >= item.quantity_needed && (
                  <> Item ini sudah <strong className="text-maroon">fully claimed</strong>.</>
                )}
              </p>
            </div>
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="min-w-0">
                <p className="font-serif italic text-xl text-ivory leading-tight">{item.name}</p>
                {item.brand && <p className="font-sans text-[10px] text-gold tracking-[0.15em] uppercase mt-0.5">{item.brand}</p>}
                <p className="font-sans text-[11px] text-ivory/40 mt-1">
                  Sisa: <span className="text-gold font-semibold">{remaining}</span> dari {item.quantity_needed}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => { setForm(p => ({ ...p, claimer_name: e.target.value })); setError(''); }}
              ...
              {item.quantity_needed > 1 && (
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >−</button>
                    <span className="font-serif text-2xl text-ivory w-8 text-center">{form.quantity}</span>
                    <button
                      onClick={() => setForm(p => ({ ...p, quantity: Math.min(remaining, p.quantity + 1) }))}
                      className="w-9 h-9 rounded-lg border border-gold/20 text-ivory flex items-center justify-center text-lg hover:border-gold/50 transition-colors"
                    >+</button>
                    <span className="font-sans text-[11px] text-ivory/40 ml-1">maks {remaining}</span>
                  </div>
                </div>
              )}
              ...
              <div>
                <label className="block font-sans text-[10px] text-gold tracking-[0.15em] uppercase mb-1.5">
                  Pesan <span className="text-ivory/30 normal-case tracking-normal font-light">opsional</span>
                </label>
                <textarea
                  value={form.message}
...
function GiftCard({ item, onClaim }) {
  ...
  return (
    ...
    >
      ...
      {isFull && (
        <div className="absolute inset-0 z-[3] bg-white/80 backdrop-blur-[2px] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="font-serif italic text-maroon text-lg">Fully Claimed</p>
      ...
      <div className="p-5">
        {item.brand && (
          <p className="font-sans font-light text-[10px] text-gold tracking-[0.15em] uppercase mb-1">{item.brand}</p>
        )}
        <h3 className="font-serif italic text-[22px] text-maroon mb-2 font-normal leading-tight">{item.name}</h3>
        {item.description && (
          <p className="font-sans font-light text-[12px] text-charcoal/65 leading-[1.7] mb-2">{item.description}</p>
        )}
        {item.price_range && (
          <p className="font-sans font-light text-[11px] text-charcoal/45 mb-3">{item.price_range}</p>
        )}

        {/* Progress */}
        <ProgressBar claimed={item.quantity_claimed} needed={item.quantity_needed} />
...
const GiftRegistry = () => {
  ...
  const handleClaimSuccess = (itemId, newClaimed) => {
    setItems(prev => prev.map(i => i.id === itemId ? { ...i, quantity_claimed: newClaimed } : i));
  };
          ...
          <div className="mb-16 obs-hide obs-up" style={{ animationDelay: '100ms' }}>
            ...
            <div className="flex flex-wrap gap-6 justify-center">
              ...
              <div className="bg-white border border-gold/30 rounded p-[28px_32px] min-w-[240px] relative flex flex-col items-start">
                ...
                <p className="font-sans font-light text-[12px] text-charcoal/60 mb-4">{settings?.groom_nickname || 'Benjamin'} &amp; {settings?.bride_nickname || 'Angelin'}</p>
                <button
                  ...
                  className={`font-sans font-light text-[10px] tracking-[0.15em] uppercase transition-colors cursor-pointer rounded-sm p-[6px_16px] border
                    ${copiedBank === 'bca' ? 'text-maroon border-maroon' : 'text-gold border-gold/40 hover:bg-gold/10'}`}
                >
              ...
              <div className="bg-white border border-gold/30 rounded p-[28px_32px] min-w-[240px] relative flex flex-col items-start">
                ...
                <button
                  ...
                  className={`font-sans font-light text-[10px] tracking-[0.15em] uppercase transition-colors cursor-pointer rounded-sm p-[6px_16px] border
                    ${copiedBank === 'mandiri' ? 'text-maroon border-maroon' : 'text-gold border-gold/40 hover:bg-gold/10'}`}
                >
          ...
          <div className="mt-[60px]">
            ...
            {loadingItems ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                {[1,2,3].map(i => (
                  <div key={i} className="rounded border border-gold/10 overflow-hidden animate-pulse">
