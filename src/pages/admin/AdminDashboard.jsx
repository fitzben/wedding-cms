...
const API = {
  dashboard: () =>
    fetch(`${BASE_URL}/api/admin/dashboard`, { headers: getHeaders() }).then(r => r.json()),
};
    ...
    <div className={`rounded-2xl border p-5 flex flex-col gap-3 ${color}`}>
      ...
      <div>
        <div className="text-2xl font-bold tracking-tight">
          {loading ? <span className="inline-block w-12 h-6 bg-current opacity-10 rounded animate-pulse" /> : value}
        </div>
        <div className="text-xs font-semibold uppercase tracking-wider opacity-60 mt-0.5">{label}</div>
...
function CountdownCard({ settings, loading }) {
  const targetDate = settings?.countdown_override_date || (
    settings?.countdown_target === 'hm' ? settings?.hm_date : settings?.resepsi_date
  );
  const timeLeft = useCountdown(targetDate);
  const eventName = settings?.countdown_target === 'hm' ? 'Holy Matrimony' : 'Resepsi';
  ...
  return (
    <div className="bg-gray-900 text-white rounded-2xl border border-gray-800 p-6 flex flex-col gap-4">
      <div>
        <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-1">Countdown to</div>
        <div className="text-base font-bold">{eventName}</div>
        {targetDate && <div className="text-xs text-gray-400 mt-0.5">{formatDate(targetDate)}</div>}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-sm text-gray-500">Set event date in Settings → Wedding</div>
      ) : timeLeft?.passed ? (
        <div className="text-emerald-400 font-semibold text-sm">🎉 The event has passed!</div>
      ) : timeLeft ? (
        <div className="flex gap-2">
          {[
            ...
            <div key={u} className="flex-1 bg-gray-800 rounded-xl p-3 text-center min-w-0">
              <div className="text-2xl font-bold tabular-nums">{pad(v)}</div>
              <div className="text-[10px] text-gray-500 mt-0.5 font-semibold tracking-widest">{u}</div>
            </div>
          ))}
        </div>
      ) : null}
...
function RSVPBreakdown({ rsvp, loading }) {
  const { attending = 0, not_attending = 0, maybe = 0, total_guests = 0, total_pax = 0 } = rsvp || {};
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{total} respons diterima</p>
        </div>
        {!loading && total > 0 && (
          <div className="text-right">
            <div className="text-2xl font-bold text-emerald-600">{attendPct}%</div>
        ...
        <>
          ...
          <div className="mt-4 pt-4 border-t border-gray-50 grid grid-cols-2 gap-3 text-center">
            <div className="bg-gray-50 rounded-xl py-2">
              <div className="text-sm font-bold text-gray-900">{total_guests}</div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wide">Guests responded</div>
            </div>
            <div className="bg-gray-50 rounded-xl py-2">
              <div className="text-sm font-bold text-gray-900">{total_pax}</div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wide">Total pax coming</div>
            </div>
          </div>
        </>
      )}
...
function GuestBreakdown({ guests, loading }) {
  const { by_category = {}, by_importance = {}, by_priority = {}, invite_sent = 0, not_sent = 0 } = guests || {};

  const catColors = { family: 'bg-purple-100 text-purple-700', colleague: 'bg-blue-100 text-blue-700', friend: 'bg-emerald-100 text-emerald-700' };
  const impColors = { vvip: 'bg-yellow-100 text-yellow-700', vip: 'bg-orange-100 text-orange-700', normal: 'bg-gray-100 text-gray-600' };
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {/* By Category */}
          <div>
            <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">By Category</div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(by_category).map(([k, v]) => (
                <div key={k} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold ${catColors[k] || 'bg-gray-100 text-gray-600'}`}>
                  <span className="capitalize">{k}</span>
                  <span className="font-bold opacity-70">{v}</span>
                </div>
              ))}
              {!Object.keys(by_category).length && <span className="text-xs text-gray-400">—</span>}
            </div>
          </div>

          {/* By Importance */}
          <div>
            <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">By Importance</div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(by_importance).map(([k, v]) => (
                <div key={k} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold ${impColors[k] || 'bg-gray-100 text-gray-600'}`}>
                  <span className="uppercase">{k}</span>
                  <span className="opacity-70">{v}</span>
                </div>
              ))}
              {!Object.keys(by_importance).length && <span className="text-xs text-gray-400">—</span>}
            </div>
          </div>

          {/* Invite status */}
          <div>
            <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Invite Sent</div>
            <div className="flex gap-2">
              <div className="flex-1 bg-emerald-50 border border-emerald-100 rounded-xl p-2.5 text-center">
                <div className="text-base font-bold text-emerald-700">{invite_sent}</div>
                <div className="text-[10px] text-emerald-600 uppercase tracking-wide">Sent</div>
              </div>
              <div className="flex-1 bg-gray-50 border border-gray-100 rounded-xl p-2.5 text-center">
                <div className="text-base font-bold text-gray-600">{not_sent}</div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        {wishes?.length > 0 && <span className="text-xs text-gray-400">{wishes.length} terbaru</span>}
      </div>
            ...
            <div key={w.id || i} className="flex gap-3 items-start">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-semibold text-gray-900 truncate">{w.name || w.guest_name}</span>
                  <span className="text-[10px] text-gray-400 flex-shrink-0">{timeAgo(w.created_at)}</span>
                </div>
                <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{w.message || w.wish}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {!loading && giftSummary && (
            <p className="text-xs text-gray-400 mt-0.5">{formatIDR(giftSummary.total_amount)} total transfer</p>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2.5">
          {gifts.slice(0, 5).map((g, i) => (
            <div key={g.id || i} className="flex items-center justify-between gap-3 py-2 border-b border-gray-50 last:border-0">
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-lg flex-shrink-0">{g.type === 'bank_transfer' ? '🏦' : '🎁'}</span>
                <div className="min-w-0">
                  <div className="text-xs font-semibold text-gray-900 truncate">{g.sender_name}</div>
                  <div className="text-[10px] text-gray-400">{timeAgo(g.created_at)}</div>
                </div>
              </div>
...
function FeatureStatus({ settings, loading }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      {loading ? (
        <div className="space-y-2">{features.map((_, i) => <div key={i} className="h-10 bg-gray-100 rounded-xl animate-pulse" />)}</div>
      ) : (
        <div className="space-y-2">
          {features.map(f => {
            ...
            const badgeCls = f.inverted
              ? (active ? 'bg-red-50 text-red-600 border-red-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100')
              : (active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-gray-50 text-gray-400 border-gray-100');
            const badgeLabel = f.inverted ? (active ? 'ACTIVE' : 'OFF') : (active ? 'ON' : 'OFF');
            return (
              <div key={f.key} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <span className={`text-[10px] font-bold px-2 py-1 rounded-full border uppercase tracking-wide ${badgeCls}`}>
                  {badgeLabel}
                </span>
              </div>
            );
...
function WeddingBanner({ settings, loading }) {
  ...
  if (loading) return (
    <div className="h-20 bg-gray-100 rounded-2xl animate-pulse mb-6" />
  );
    ...
    <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white rounded-2xl px-7 py-5 mb-6 flex items-center justify-between gap-4 overflow-hidden relative">
      ...
      {(dateRes || venueRes) && (
        <div className="relative text-right flex-shrink-0">
          {dateRes && <div className="text-sm font-semibold">{formatDate(dateRes)}</div>}
          {venueRes && <div className="text-xs text-gray-400 mt-0.5">{venueRes}</div>}
        </div>
      )}
    </div>
  );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              ...
              {lastRefresh && <span className="ml-2 text-gray-300">· Updated {timeAgo(lastRefresh)}</span>}
            </p>
          </div>
          <button
            onClick={load}
