...
const API = {
  // Sections
  getSections: () => apiClient.get("/api/admin/gallery/sections"),
  createSection: (body) => apiClient.post("/api/admin/gallery/sections", body),
  updateSection: (id, body) =>
    apiClient.put(`/api/admin/gallery/sections/${id}`, body),
  deleteSection: (id) => apiClient.delete(`/api/admin/gallery/sections/${id}`),
  reorderSections: (ids) =>
    apiClient.post("/api/admin/gallery/sections/reorder", { ids }),

  // Media
  getMedia: (sectionId) =>
    apiClient.get(`/api/admin/gallery/media?section_id=${sectionId}`),
  getUploadUrl: (filename, contentType, sectionId) =>
    apiClient.post("/api/admin/gallery/upload-url", {
      filename,
      content_type: contentType,
      section_id: sectionId,
    }),
  confirmUpload: (body) => apiClient.post("/api/admin/gallery/media", body),
  updateMedia: (id, body) =>
    apiClient.put(`/api/admin/gallery/media/${id}`, body),
  deleteMedia: (id) => apiClient.delete(`/api/admin/gallery/media/${id}`),
  reorderMedia: (sectionId, ids) =>
    apiClient.post("/api/admin/gallery/media/reorder", {
      section_id: sectionId,
      ids,
    }),
  setCover: (sectionId, mediaId) =>
    apiClient.put(`/api/admin/gallery/sections/${sectionId}/cover`, {
      media_id: mediaId,
    }),
};
...
// ─── Caption Edit Modal ───────────────────────────────────────────────────────
function CaptionModal({ media, onClose, onSave }) {
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Prewedding Photos"
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <div
              onClick={() =>
                setForm((p) => ({ ...p, accepts_video: !p.accepts_video }))
              }
              className={`relative w-10 h-5 rounded-full transition-colors ${form.accepts_video ? "bg-gray-900" : "bg-gray-200"}`}
            >
              <span
                className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${form.accepts_video ? "translate-x-5" : ""}`}
              />
            </div>
            <span className="text-sm text-gray-700 font-medium">
              Allow video uploads in this section
            </span>
          </label>
        </div>
    ...
    <div>
      ...
      >
        <input
          ...
          type="file"
          multiple
          accept={section.accepts_video ? "image/*,video/*" : "image/*"}
          className="hidden"
        ...
        <p className="text-sm text-gray-500 font-medium">
          Drop {section.accepts_video ? "photos & videos" : "photos"} here or{" "}
        ...
        <div className="mt-3 space-y-2">
          {progress.map((p) => (
            ...
            >
              ...
              <span
                className={`text-xs font-semibold ${p.error ? "text-red-500" : p.done ? "text-emerald-600" : "text-gray-500"}`}
              >
  ...
  return (
    <div
      ...
      className={`relative group rounded-xl overflow-hidden border-2 transition-all
        ${item.id === section.cover_media_id ? "border-amber-400" : "border-transparent hover:border-gray-200"}
        ${isDraggingAny && !isDragging ? "cursor-grabbing" : ""}
      `}
    >
      {/* Drag handle */}
      <div
      ...
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex flex-col justify-between p-2">
        ...
        <div className="flex items-start justify-between opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="flex gap-1 flex-wrap">
            {item.media_type === "video" && (
              <span className="bg-black/70 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md">
                VIDEO
              </span>
            )}
            {item.id === section.cover_media_id && (
              <span className="bg-amber-400 text-amber-900 text-[10px] font-bold px-1.5 py-0.5 rounded-md">
                COVER
              </span>
            )}
          </div>
      ...
      {/* Caption indicator (when not hovering) */}
      {item.caption && (
    ...
    <>
      ...
      >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {items.map((item) => (
              <SortableMediaItem
                ...
                onCaption={(m) => setCaptionModal({ open: true, media: m })}
        ...
        <DragOverlay adjustScale={false}>
          {activeItem ? (
            <div
              className="rounded-xl overflow-hidden shadow-2xl ring-2 ring-gray-900/20"
              style={{ aspectRatio: "1", width: "100%", opacity: 0.9 }}
            >
              <img
...
function SectionPanel({ section, onEdit, onDelete, push }) {
  ...
  const loadMedia = useCallback(async () => {
    ...
    try {
      const data = await API.getMedia(section.id);
      setMedia(data.media || data || []);
    } catch {
      push("Failed to load media", "error");
    } finally {
...
export const AdminGallery = () => {
  ...
  return (
    <>
      ...
      {sectionModal.open && (
        <SectionModal
          key={sectionModal.initial?.id || "new"}
        ...
        <div className="px-6 md:px-8 py-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              ...
              Upload, organize, and reorder photos & videos per section
            </p>
          </div>
          <button
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400 text-sm mb-4">
                Create a section to start organizing your gallery
              </p>
              <button
                onClick={() => setSectionModal({ open: true, initial: null })}
