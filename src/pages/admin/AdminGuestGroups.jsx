...
const API = {
  ...
  create: (body) =>
    fetch(`${BASE_URL}/api/admin/guest-groups`, {
      method: 'POST', headers: getHeaders(), body: JSON.stringify(body),
    }).then(r => r.json()),
  update: (id, body) =>
    fetch(`${BASE_URL}/api/admin/guest-groups/${id}`, {
  ...
  guestCount: (groupId) =>
    fetch(`${BASE_URL}/api/admin/guests?guest_group_id=${groupId}&limit=1`, { headers: getHeaders() })
...
function Toast({ toasts }) {
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
      {toasts.map(t => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg text-sm font-medium pointer-events-auto transition-all duration-300
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-7 w-full max-w-sm mx-4 border border-gray-100">
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onCancel} className="px-4 py-2 rounded-xl border border-gray-200 text-gray-700 text-sm font-semibold hover:bg-gray-50 transition-all">
            Batal
          </button>
          <button onClick={onConfirm} className="px-4 py-2 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-all shadow-sm">
            Hapus
          </button>
        </div>
      </div>
...
function GroupModal({ open, onClose, onSave, initial }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? { name: initial.name || '', description: initial.description || '', default_event_access: initial.default_event_access || 'both' } : EMPTY_FORM);
      setError('');
  ...
  const handleSubmit = async () => {
    if (!form.name.trim()) { setError('Nama group harus diisi'); return; }
    setSaving(true);
    try {
      await onSave(form);
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 border border-gray-100 overflow-hidden">
        ...
        <div className="px-7 pt-7 pb-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">{initial ? 'Edit Group' : 'Tambah Guest Group'}</h2>
            <p className="text-gray-400 text-sm mt-0.5">
              {initial ? 'Ubah nama atau deskripsi group' : 'Contoh: Keluarga Mama, Rekan Kerja Papa'}
            </p>
          </div>
        ...
        <div className="px-7 py-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">
              Akses Acara
              <span className="ml-1.5 text-gray-300 font-normal normal-case tracking-normal">default untuk semua anggota group</span>
            </label>
              ...
              {[
                ...
                >
                  <input
                    ...
                    onChange={() => setForm(p => ({ ...p, default_event_access: value }))}
                    className="sr-only"
                  />
                  {icon} {label}
                </label>
            ...
            <p className="text-xs text-gray-400 mt-1.5">
              Tamu dalam group ini akan diundang sesuai pilihan ini, kecuali ada override per-tamu.
            </p>
          </div>
        </div>
        ...
        <div className="px-7 pb-7 flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-sm font-semibold hover:bg-gray-50 transition-all">
            Batal
          </button>
          <button
          ...
          >
            ...
            {saving ? 'Menyimpan…' : initial ? 'Simpan Perubahan' : 'Tambah Group'}
          </button>
        </div>
      </div>
    </div>
...
export const AdminGuestGroups = () => {
  ...
  const openCreate = () => { setEditGroup(null); setModalOpen(true); };
  const openEdit   = (g)  => { setEditGroup(g);  setModalOpen(true); };
  ...
  const handleDelete = (group) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        if (!res.ok && res.status !== 204) { push('Gagal menghapus group', 'error'); return; }
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-8 border-b border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">
                {groups.length} group · Kelompokkan tamu berdasarkan hubungan atau asal
              </p>
            </div>
            {isAdmin && (
              <button
        ...
        <div className="p-8">
          {error && (
            <div className="mb-5 p-4 bg-red-50 text-red-600 rounded-xl border border-red-100 text-sm">{error}</div>
          )}
          ...
          {loading ? (
            ...
            <div className="py-16 flex flex-col items-center gap-4 text-gray-400">
              ...
              <div className="text-center">
                <p className="font-semibold text-gray-600">Belum ada guest group</p>
                <p className="text-sm text-gray-400 mt-1">Buat group untuk mengelompokkan tamu</p>
              </div>
              {isAdmin && (
                <button
                  onClick={openCreate}
...
function GroupCard({ group, isAdmin, onEdit, onDelete }) {
  ...
  const colorIdx = group.name.split('').reduce((a, c) => a + c.charCodeAt(0), 0) % colors.length;
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {group.description && (
              <p className="text-xs text-gray-400 mt-0.5 line-clamp-2 leading-relaxed">{group.description}</p>
            )}
          </div>
        </div>
...
// ─── Event Access Badge ───────────────────────────────────────────────────────
function EventAccessBadge({ access, small = false }) {
  const map = {
